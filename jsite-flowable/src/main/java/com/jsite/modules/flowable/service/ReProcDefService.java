...
//		InputStream resourceAsStream = repositoryService.getProcessDiagram(procDefId);
    ...
    public Page<RuExecution> runningList(Page<RuExecution> page, String procInsId, String procDefKey) {
        ...
        ExecutionQuery processInstanceQuery = runtimeService.createExecutionQuery();
        ...
        List<Execution> procInsList = processInstanceQuery.listPage(page.getFirstResult(), page.getMaxResults());
        for (Execution procIns : procInsList) {
            if (procIns.getActivityId() != null) {
                Task task = taskQuery.taskDefinitionKey(procIns.getActivityId()).singleResult();
                ...
                ruExecution.setName(task.getName());
